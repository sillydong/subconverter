...
std::vector<std::string> splitN(const std::string &s, const std::string &separator, const int &count);
...
std::string join(InputIt first, InputIt last, const std::string &delimiter)
{
    ...
    if (std::next(first) == last)
        return *first;
    return std::accumulate(std::next(first), last, *first, [&](const std::string &a, const std::string &b)
